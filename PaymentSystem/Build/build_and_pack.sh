...
mkdir -p "$output_path"/pb
# copiar script de instalação de docker e docker compose
cp -f deploy_and_run.sh "$output_path"/pb
...
docker save paymentsystem/webapi mongo-express mongo -o "$output_path"/pb/payment-system-images.tar.gz

echo "------------------- Copying docker-compose of application... -------------------"
cp -f .env "$output_path"/pb
cp -f mongo-init.js "$output_path"/pb
cp -f docker-compose.yml "$output_path"/pb

echo "------------------- Compressing final file with all backend application content... -------------------"
cd "$output_path"/pb && zip -r "$output_path"/payment_base.zip . && cd - || exit 1

echo "------------------- Cleaning output path... -------------------"
rm -rf "$output_path"/pb